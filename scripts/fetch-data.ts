...
function restoreFromCache(): boolean {
  if (existsSync(ARCHIVE_OUTPUT) && existsSync(MEDIUM_OUTPUT)) {
    console.warn('Using existing static data cache.');
    return true;
  }
  if (existsSync(ARCHIVE_CACHE) && existsSync(MEDIUM_CACHE)) {
    ...
    console.warn('Using cached data from previous build output.');
    return true;
  }
  return false;
}
